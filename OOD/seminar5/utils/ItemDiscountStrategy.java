...
/**
 * A discount strategy that calculates the discount based on specific item in the sale.
...
public class ItemDiscountStrategy implements DiscountStrategy {
     ...
     */
    public ItemDiscountStrategy(int customerId) {
     ...
     * @return The discount amount applied based on items in sale and customerId.
     */
    @Override
    public double getDiscount(double totalPrice, List<SaleItems> items, DiscountDB discountDB) {
        return discountDB.getItemDiscount(items, customerId);
    }
}